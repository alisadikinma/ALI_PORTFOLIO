...
<section id="gallery" class="w-full max-w-screen-xl mx-auto px-3 sm:px-4 py-6 flex flex-col items-center gap-6 sm:gap-10">
    ...
    <div>
    <div class="relative group rounded-lg bg-slate-900 outline outline-1 outline-slate-500 overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-md cursor-pointer" onclick="openGalleryModal({{ $row->id_galeri }}, '{{ addslashes($row->nama_galeri) }}')">
    @if($row->thumbnail)
    ...
    @endif
        ...
        {
            @foreach ($galeri->where('status', 'Active') as $gallery)
            "{{ $gallery->id_galeri }}": {
                "id": {{ $gallery->id_galeri }},
                "name": "{{ addslashes($gallery->nama_galeri) }}",
                "items": [
                    @foreach ($gallery->galleryItems->where('status', 'Active') as $item)
                    {
                        ...
                        "file_url": "{{ $item->file_name ? asset('file/galeri/' . $item->file_name) : '' }}"
                    }@if(!$loop->last),@endif
                    @endforeach
                ]
            }@if(!$loop->last),@endif
